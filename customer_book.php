...
if(isset($_SESSION['admin_login_id']) && isset($_GET['bid'])){
...
<div class="container-fluid">
	<form id="manage_book">
		<div class="col-md-12">
			<h2 style="color:black;">Reservation Details</h1>
			...
			<?php if(($count < $meta['availability']) || isset($_SESSION['admin_login_id'])): ?>
			...
			<div class="form-group mb-2">
				<label for="qty" class="control-label">Number of seats to book</label>
				<input type="number" maxlength="4" class="form-control text-right" id="qty" name="qty" required value="<?php echo isset($bmeta['qty']) ? $bmeta['qty'] : '' ?>">
			</div>
			...
			<div class="form-group mb-2">
				...
				<input type="number" maxlength="4" class="form-control text-right" id="cardnumber" name="cardnumber" required value="<?php echo isset($bmeta['cardnumber']) ? $bmeta['cardnumber'] : '' ?>">
			</div>
			
			
			<div class="form-group mb-2">
				<label for="name" class="control-label">Full Name</label>
				<input type="text" class="form-control" id="name" name="name" required value="<?php echo isset($bmeta['name']) ? $bmeta['name'] : '' ?>">
			</div>

			<div class="row">
			<div class="col-md-6">
			<div class="form-group mb-2">
				<label for="expirationdate" class="control-label">Expiration (mm/yy)</label>
				<input type="text" pattern="[0-9]*" maxlength="4" class="form-control text-right" id="expirationdate" name="expirationdate" value="<?php echo isset($bmeta['expirationdate']) ? $bmeta['expirationdate'] : '' ?>">
			</div>
			</div>
			...
			<div class="form-group mb-2">
				...
				<input type="password" maxlength="4" class="form-control text-right" id="securitycode" name="securitycode" value="<?php echo isset($bmeta['securitycode']) ? $bmeta['securitycode'] : '' ?>">
			</div>
			</div>
			</div>
		...
		$.ajax({
			...
			success:function(resp){
				...
				if(resp.status == 1){
    				end_load()
    				$('.modal').modal('hide')
    				alert_toast('Your data has been successfully saved :)','success');
    				if('<?php echo !isset($_SESSION['admin_login_id']) ?>' == 1){
    				$('#book_modal .modal-body').html('<div class="text-center"><p>Your seat reservation has been made. <br><strong><h3>'+resp.ref+'</h3></strong></p><small>Reference Number</small><br/><small>Copy or Capture your Reference number </small><br><br> Thank You :)</div>')
